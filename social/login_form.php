   ...
   isset($_POST['password_text']))
{
    ...
    if(!(empty($un) || empty($pw)))
    {
        ...
        if($result=="success")
        {
           //User can now login.
            ...
            $_SESSION['user_id']=  getUserInfo($un,'id');
            $_SESSION['user_un'] = $un;
            $_SESSION['user_fn'] = getUserInfo($un,'firstname');
            $_SESSION['user_ln'] = getUserInfo($un,'lastname');

            $_SESSION['user_em'] = getUserInfo($un,'email');

            $_SESSION['user_ph'] = getUserInfo($un,'phnumber');
            echo "Logging in...<br><br>";
           header("refresh:0;url=http://localhost/social");
        }
        else
        {
            echo $result;
    ...
    {
        echo "Enter details and then submit!";
    }
}
function performLogin($un,$pw)
   {
       $result = Fetch("select password from users where username='$un'");
